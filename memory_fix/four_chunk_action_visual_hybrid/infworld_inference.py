...
def select_history_windows(video_buffer, seed_frame_count, frame_action_history):
    ...
    for idx, candidate in enumerate(candidates):
        ...
        candidate["visual_score"] = 0.75 * candidate["motion"] + 0.25 * candidate["recency"]
        if candidate["has_actions"]:
            ...
            candidate["generated_score"] = candidate["visual_score"]
    ...
    midpoint = total_frames / 2.0
    coverage_pick = _choose_best_candidate(
        candidates,
        selected,
        key_fn=lambda item: -abs(((item["start"] + item["end"]) / 2.0) - midpoint),
        max_overlap=max_overlap,
    )
    if coverage_pick is not None:
        coverage_pick["selected"] = True
        selected.append(coverage_pick)

    for _ in range(1):
        generated_pick = _choose_best_candidate(
            candidates,
            selected,
            key_fn=lambda item: item["generated_score"],
            max_overlap=max_overlap,
        )
        if generated_pick is None:
            break
        generated_pick["selected"] = True
        selected.append(generated_pick)
    ...
    if selected:
        ...
        print(f"[MemoryFix/four_chunk_action_visual_hybrid] Selected windows: {debug_segments}")
