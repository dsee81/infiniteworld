...
class WanModel(nn.Module):
    ...
    def forward(
        ...
        COMPRESSION_RATE = 4
        MAX_T_OUT = 16
        TARGET_T_MID = MAX_T_OUT * COMPRESSION_RATE # 64
        W_IN = 64
        W_OUT_PER_CHUNK = W_IN // COMPRESSION_RATE # 16
        TARGET_N_CHUNKS = 4 # 确保 T_mid = 64
        ...
        if T_in <= TARGET_T_MID: 
            ...
            # 情况 B: T_in > 64，滑动窗口 + 二次压缩
            
            # --- Step 1: 滑动窗口分块编码 (T_in -> T_mid=64) ---
            
            # 计算步长 S，确保 4 个 Chunk 覆盖 T_in
            ...
            # 拼接中间序列 T_mid (T_mid = 64)
            image_cond = torch.cat(latent_chunks, dim=2)
            T_mid = image_cond.shape[2] 
            
            # --- Step 2: 二次压缩 (T_mid=64 -> T_out=16) ---
            
            if T_mid > MAX_T_OUT:
                # 此时 T_mid = 64，是 4 的倍数，直接编码即可
                image_cond = self.latent_encoder(image_cond)
                # T_out = 16
