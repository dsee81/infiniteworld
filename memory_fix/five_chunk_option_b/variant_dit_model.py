...
def flash_attention(
    ...
    if (version is None or version == 3) and FLASH_ATTN_3_AVAILABLE:
        ...
        if window_size != (-1, -1):
            warnings.warn(
                'FlashAttention is unavailable; falling back to PyTorch SDPA without local windowing.'
            )
        q_sdpa = q_orig.to(v.dtype)
        k_sdpa = k_orig.to(v.dtype)
        v_sdpa = v_orig.to(v.dtype)
        if q_scale is not None:
            q_sdpa = q_sdpa * q_scale

        outputs = []
        for i in range(b):
            q_len_i = lq if q_lens is None else int(q_lens[i].item())
            k_len_i = lk if k_lens is None else int(k_lens[i].item())
            q_i = q_sdpa[i, :q_len_i].transpose(0, 1)
            k_i = k_sdpa[i, :k_len_i].transpose(0, 1)
            v_i = v_sdpa[i, :k_len_i].transpose(0, 1)
            ...
            if q_len_i < lq:
                out_i = torch.cat(
                    [out_i, out_i.new_zeros(lq - q_len_i, out_i.size(1), out_i.size(2))],
                    dim=0,
                )
...
class WanModel(nn.Module):
    ...
    def forward(
        ...
        if T_in <= TARGET_T_MID: 
            ...
            # 情况 B: T_in > 80，使用最新的 5 个 64 帧窗口 + 二次压缩
            # 将历史尾部对齐到 5 * 64 = 320 帧，不足时在前侧补 0，然后切成 5 个连续 chunk。
            total_tail_frames = TARGET_N_CHUNKS * W_IN
            tail = image_cond[:, :, max(T_in - total_tail_frames, 0):, :, :]
            front_padding_len = total_tail_frames - tail.shape[2]
            if front_padding_len > 0:
                tail = F.pad(tail, (0, 0, 0, 0, front_padding_len, 0))
